...
private
      ...
      DATA MP,LP,GTOL,STPMIN,STPMAX/6,6,9.0E-01_dp,1.0E-20_dp,1.0E+20_dp/
...
!  
!  
!      DIAG    is a real(dp) array of length N. If DIAGCO=.TRUE.,
...
!  
!  
!      EPS     is a positive real(dp) variable that must be set by
...
!  
!      XTOL    is a  positive real(dp) variable that must be set by
...
!              is less than XTOL.
!  
!      W       is a real(dp) array of length N(2M+1)+2M used as
...
!  
!     GTOL is a real(dp) variable with default value 0.9, which
...
!  
!     STPMIN and STPMAX are non-negative real(dp) variables which
...
!      - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
! 
      real(dp) GTOL,ONE,ZERO,GNORM,STP1,FTOL,STPMIN,STPMAX,STP,YS,YY,SQ,YR,BETA,XNORM
...
!
   ...
   50 continue
      return
      end subroutine daxpylinlin
! 
! 
!    ----------------------------------------------------------
! 
      real(dp) function ddotlin(n,dx,incx,dy,incy)
!
!     forms the dot product of two vectors.
!     uses unrolled loops for increments equal to one.
!     jack dongarra, linpack, 3/11/78.
!
      real(dp) dx(1),dy(1),dtemp
      integer i,incx,incy,ix,iy,m,mp1,n
!
      ddotlin = 0.0_dp
      dtemp = 0.0_dp
...
! 
      ...
      INTEGER N,MAXFEV,INFO,NFEV,LP,MP
...
! 
      ...
      SUBROUTINE MCSTEP(STX,FX,DX,STY,FY,DY,STP,FP,DDP,BRACKT,STPMIN,STPMAX,INFO)
      INTEGER INFO
      real(dp) STX,FX,DX,STY,FY,DY,STP,FP,DDP,STPMIN,STPMAX
...
!      THE SUBROUTINE STATEMENT IS
! 
!        SUBROUTINE MCSTEP(STX,FX,DX,STY,FY,DY,STP,FP,DDP,BRACKT,
!                         STPMIN,STPMAX,INFO)
! 
!      WHERE
! 
...
! 
!        STP, FP, AND DDP ARE VARIABLES WHICH SPECIFY THE STEP,
...
! 
      ELSE IF (ABS(DDP) .LT. ABS(DX)) THEN
         INFO = 3
         BOUND = .TRUE.
         THETA = 3*(FX - FP)/(STP - STX) + DX + DDP
         S = MAX(ABS(THETA),ABS(DX),ABS(DDP))
...
! 
         GAMMA = S*SQRT(MAX(0.0_dp,(THETA/S)**2 - (DX/S)*(DDP/S)))
         IF (STP .GT. STX) GAMMA = -GAMMA
         P = (GAMMA - DDP) + THETA
